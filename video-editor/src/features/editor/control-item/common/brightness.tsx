...
const Brightness = ({
  ...
  const { value: storeValue, isMixed } = useMixedValue<number>(
    targetIds,
    (item) => item.details?.brightness ?? 100
  );
  ...
  const commit = (num: number) => {
    dispatchGroupEdit(targetIds, { brightness: num });
  ...
  const handleKeyDown = (e: KeyboardEvent<HTMLInputElement>) => {
    ...
    if (!Number.isNaN(num) && num >= 0 && num <= 200) commit(num);
  };
      ...
      <div className="w-full flex gap-2">
        <Input
          ...
          onChange={(e) => {
            ...
            if (
              raw === "" ||
              (!Number.isNaN(Number(raw)) && Number(raw) >= 0 && Number(raw) <= 200)
            ) {
              setLocalValue(raw);
            }
          }}

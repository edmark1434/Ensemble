...
function IssueViolationModal({ onClose, onIssued }: { onClose: () => void; onIssued: () => void }) {
  ...
  const submit = async () => {
    ...
    setSaving(true);
    try {
      await api.post("/api/moderator/forum/restrictions/violations", {
        ...
        <div className="space-y-3">
          ...
          <label className="flex flex-col gap-1 text-xs text-zinc-500">
            ...
            <input
              ...
              placeholder="e.g. Spamming forum discussions"
              className="rounded-lg border border-white/10 bg-[#14151c] px-3 py-2 text-sm text-white outline-none"
            />
          </label>
          <label className="flex flex-col gap-1 text-xs text-zinc-500">
          ...
          <button
            ...
            onClick={() => void submit()}
            className="w-full rounded-xl bg-violet-500/90 px-4 py-2 text-sm font-medium text-white hover:bg-violet-500 disabled:opacity-50"
          >
...
export default function ForumRestrictions() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const res = await api.get("/api/moderator/forum/restrictions");
  ...
  const setRestriction = async (accountId: number | string, status: "suspended" | "banned" | "active") => {
    try {
      await api.patch(`/api/moderator/forum/restrictions/accounts/${accountId}`, { status });
    ...
    <main className="relative z-10 min-h-screen px-6 py-8 md:pl-[260px] md:px-10" style={{ animation: "fadeIn 420ms ease" }}>
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-zinc-500">Restrict accounts that break forum rules.</p>
        </div>
        <button
          type="button"
          onClick={() => setShowIssueModal(true)}
          className="flex items-center gap-2 rounded-xl bg-violet-500/90 px-4 py-2 text-sm font-medium text-white hover:bg-violet-500"
        >

    ...
    <aside className="fixed inset-y-0 left-0 z-20 hidden w-72 border-r border-white/10 bg-black/40 backdrop-blur-xl md:flex md:flex-col">
      <div className="flex items-center gap-4 border-b border-white/10 px-6 py-5">
        ...
        <div>
          ...
          <p className="text-[13px] font-semibold leading-tight tracking-wide text-white">Jobs &amp; Gigs Moderator</p>
        </div>
      </div>
      ...
      <nav className="flex-1 overflow-y-auto px-3 py-4">
        <ul className="space-y-1.5">
          {navItems.map(({ label, icon: Icon, to }) => (
            <li key={label}>
              <NavLink to={to} end={to === "/moderator/jobs"} className={({ isActive }) => navClassName(isActive)}>

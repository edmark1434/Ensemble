...
export default function TicketDetailModal({
  ...
  const load = async () => {
    setLoading(true);
    try {
      const res = await api.get(`/api/moderator/tickets/${ticketId}`);
      if (res.data?.success) {
  ...
  const saveChanges = async () => {
    setSaving(true);
    try {
      await api.patch(`/api/moderator/tickets/${ticketId}`, {
        status,
        priority,
  ...
  const escalateToAdmin = async () => {
    ...
    try {
      ...
      showSuccessToast("Ticket escalated to Admin");
      await load();
      onUpdated();
    } catch {
  ...
  const sendReply = async () => {
    ...
    setSaving(true);
    try {
      await api.post(`/api/moderator/tickets/${ticketId}/messages`, {
        body: reply.trim(),

...
const DashboardSkeletonLoader: React.FC = () => (
  <div className="mx-auto max-w-7xl p-6 md:p-8 space-y-8 animate-pulse">
    {/* Tabs Skeleton */}
    <div className="flex space-x-1 p-1 bg-white/5 rounded-xl w-max border border-white/10">
    ...
    <div className="space-y-4">
        {Array.from({ length: 3 }).map((_, i) => (
            <div key={i} className="h-32 w-full rounded-2xl border border-white/10 bg-white/5 p-6" />
        ))}
    </div>
  </div>
);
            ...
            <div className="mx-auto max-w-7xl p-6 md:p-8 space-y-8 animate-fade-in">
                ...
                <div className="relative overflow-hidden rounded-2xl border border-white/10 bg-zinc-950/60 p-6 md:p-8 backdrop-blur-xl">
                    <div className="relative z-10 flex flex-col sm:flex-row justify-between sm:items-center gap-4 w-full">
                        ...
                        <div className="flex items-center gap-3 bg-white/5 border border-white/10 px-5 py-2.5 rounded-xl text-zinc-300 shadow-xl w-max">
                            ...
                            <span className="text-sm text-zinc-500 border-l border-white/10 pl-3">{currentTime.toLocaleDateString()}</span>
                        </div>
                    </div>
                </div>

                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div className="border-b border-white/10 flex gap-1 relative overflow-x-auto w-full max-w-full">
                        <button 
                        ...
                        >
                            <span className="relative z-10 flex items-center gap-2">
                                ...
                                <span className="rounded-full bg-white/10 px-2 py-0.2 text-[10px] text-zinc-300">{myTasks.length}</span>
                            </span>
                            {activeTab === 'freelancer' && (
                                <>
                                    <motion.div layoutId="dashboardTabGlow" className="absolute inset-0 bg-blue-500/5 rounded-t-lg" transition={{ duration: 0.2, ease: "easeOut" }} />
                        ...
                        >
                            <span className="relative z-10 flex items-center gap-2">
                                ...
                                <span className="rounded-full bg-white/10 px-2 py-0.2 text-[10px] text-zinc-300">{toReview.length}</span>
                            </span>
                            {activeTab === 'client' && (
                                <>
                                    <motion.div layoutId="dashboardTabGlow" className="absolute inset-0 bg-blue-500/5 rounded-t-lg" transition={{ duration: 0.2, ease: "easeOut" }} />
                        ...
                        >
                            <span className="relative z-10 flex items-center gap-2">
                                ...
                                <span className="rounded-full bg-white/10 px-2 py-0.2 text-[10px] text-zinc-300">{archivedTasks.length}</span>
                            </span>
                            {activeTab === 'archived' && (
                                <>
                                    <motion.div layoutId="dashboardTabGlow" className="absolute inset-0 bg-blue-500/5 rounded-t-lg" transition={{ duration: 0.2, ease: "easeOut" }} />
                ...
                {loading ? (
                    ...
                    <div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8 mt-6">
                            <div className="bg-[#0d0f1a]/70 border border-white/10 rounded-2xl p-5 flex items-center justify-between shadow-xl">
                                <div>
                                    <p className="text-[11px] font-bold uppercase tracking-wider text-zinc-500">Ongoing Tasks</p>
                                    <h2 className="text-3xl font-bold text-white mt-1">{ongoingCount}</h2>
                                </div>
                            ...
                            <div className="bg-[#0d0f1a]/70 border border-white/10 rounded-2xl p-5 flex items-center justify-between shadow-xl">
                                <div>
                                    <p className="text-[11px] font-bold uppercase tracking-wider text-zinc-500">Upcoming Tasks</p>
                                    <h2 className="text-3xl font-bold text-white mt-1">{waitingCount}</h2>
                                </div>
                            ...
                            <div className="bg-[#0d0f1a]/70 border border-white/10 rounded-2xl p-5 flex items-center justify-between shadow-xl">
                                <div>
                                    <p className="text-[11px] font-bold uppercase tracking-wider text-zinc-500">Completed Tasks</p>
                                    <h2 className="text-3xl font-bold text-white mt-1">{archivedTasks.length}</h2>
                                </div>
                            ...
                            <div className="bg-[#0d0f1a]/70 border border-white/10 rounded-2xl p-5 flex items-center justify-between shadow-xl">
                                <div>
                                    <p className="text-[11px] font-bold uppercase tracking-wider text-zinc-500">Remaining Milestones</p>
                                    <h2 className="text-3xl font-bold text-white mt-1">{remainingMilestonesCount}</h2>
                                </div>
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                            {/* My Tasks Column */}
                            <div className="bg-[#0d0f1a]/70 rounded-2xl border border-white/10 p-6">
                                <h3 className="text-xl font-bold text-white mb-4">My Tasks</h3>
                                {myTasks.length === 0 ? (
                                    <p className="text-sm text-zinc-500 italic">No tasks currently.</p>
                                ) : (
                                    <div className="space-y-4">
                                        {myTasks.map(task => {
                                            ...
                                            return (
                                                <div 
                                                    ...
                                                    className="flex items-center gap-4 bg-white/5 p-3 rounded-xl border border-white/10 cursor-pointer hover:bg-white/10 transition-colors"
                                                >
                                                    <img 
                                                    ...
                                                    <div className="flex-1 flex flex-col justify-center min-w-0 pr-4">
                                                        <div className="flex items-center gap-2 mb-1.5">
                                                            <h4 className="text-sm font-bold text-white truncate">{task.job_title}</h4>
                                                            <a href={`/jobs/postings/${task.job_id}`} target="_blank" rel="noreferrer" onClick={(e) => e.stopPropagation()} className="text-zinc-500 hover:text-white transition-colors shrink-0" title="View Original Job Post">
                                                    ...
                                                    <div className="flex flex-col items-end gap-1 shrink-0">
                                                        ...
                                                        <span className="text-[10px] uppercase font-bold px-2 py-0.5 rounded border bg-zinc-500/10 text-zinc-400 border-zinc-500/20">
                                                            Job
                                                        </span>
                                                    </div>
                                                </div>
                            ...
                            {/* To Review Column */}
                            <div className="bg-[#0d0f1a]/70 rounded-2xl border border-white/10 p-6">
                                <h3 className="text-xl font-bold text-white mb-4">To Review</h3>
                                {toReview.length === 0 ? (
                                    <p className="text-sm text-zinc-500 italic">No tasks to review currently.</p>
                                ) : (
                                    <div className="space-y-4">
                                        {toReview.map(task => {
                                            let computedStatus = task.contract_status;

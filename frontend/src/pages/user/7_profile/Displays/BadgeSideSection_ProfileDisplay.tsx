...
export const BadgeSideSectionSkeleton: React.FC = () => (
  <div className="rounded-2xl border border-white/10 bg-[#0b0e17]/60 p-5 space-y-3 animate-pulse">
    ...
    <div className="rounded-2xl border border-white/10 bg-[#0b0e17]/90 p-4 text-zinc-300 shadow-xl font-['Plus Jakarta Sans',sans-serif] space-y-3 relative z-20">
      ...
      <div className="flex items-center justify-between border-b border-white/5 pb-2">
        <div className="flex items-center gap-2">
          <h3 className="text-[11px] font-extrabold text-white uppercase tracking-wider">Account Badges</h3>
        </div>
        ...
        <div className="flex items-center gap-1.5 relative">
          ...
          <div className="relative group/help">
            ...
            <div className="absolute left-full top-0 ml-4 w-80 p-4 rounded-2xl border border-white/10 bg-[#070913] shadow-[0_20px_50px_rgba(0,0,0,0.8)] opacity-0 scale-95 pointer-events-none transition-all duration-200 ease-out group-hover/help:opacity-100 group-hover/help:scale-100 z-50 text-left space-y-3 origin-top-left">
              <div className="absolute right-full top-3.5 border-[6px] border-transparent border-r-[#070913]" />
              <div className="absolute right-full top-3.5 -mr-[1px] border-[6px] border-transparent border-r-white/10 -z-10" />

              <p className="text-[10px] uppercase font-black tracking-widest text-zinc-500 border-b border-white/5 pb-1.5">Badge Classification Hierarchy</p>
                  ...
                  >
                    ...
                    <div className="space-y-0.5 flex-1">
                      <div className="flex items-center justify-between">
                        <span className="font-extrabold text-white tracking-wide">{r.label}</span>
                        <span className="text-[8px] font-mono tracking-wider opacity-60" style={{ color: r.color }}>Tier 0{i + 1}</span>
                      </div>
                      <p className="text-zinc-400 font-medium leading-relaxed text-[10px]">{r.desc}</p>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <button
            ...
            className="p-1.5 rounded-lg border border-white/5 bg-white/[0.02] hover:bg-white/10 hover:text-white transition-all duration-200"
      ...
      {displayBadges.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-4 px-4 bg-white/[0.01] border border-dashed border-white/5 rounded-xl text-center space-y-1">
            ...
            {isCollapsed ? (
              ...
              >
                {displayBadges.map((b) => (
                  <div
                    key={b.id}
                    className="group relative flex items-center justify-center aspect-square p-1 rounded-xl transition-all duration-200 cursor-help border bg-[#0d111d]"
                    ...
                    onMouseLeave={(e) => {
                      ...
                      e.currentTarget.style.backgroundColor = "#0d111d";
                    }}
                  >
                    <div
                      className="bg-[#121624] border border-white/5 rounded-full flex items-center justify-center flex-shrink-0 transition-all duration-300 w-12 h-12 p-1.5"
                    ...
                    <div className="absolute left-full top-1/2 -translate-y-1/2 ml-4 w-80 p-5 rounded-2xl border border-white/10 bg-[#070913] shadow-[0_25px_60px_-10px_rgba(0,0,0,0.85)] opacity-0 scale-95 pointer-events-none transition-all duration-200 group-hover:opacity-100 group-hover:scale-100 z-50 text-left space-y-4 origin-left">
                      ...
                      <div className="flex items-center gap-4 border-b border-white/5 pb-3">
                        <div className="bg-[#121624] border border-white/10 rounded-full p-2 flex items-center justify-center flex-shrink-0 w-16 h-16 shadow-xl relative" style={{ boxShadow: `0 0 20px ${b.glowColor}30` }}>
                          <img src={b.icon} alt={b.name} className="w-full h-full object-contain filter drop-shadow-[0_4px_8px_rgba(0,0,0,0.6)]" />
                        </div>
                        <div className="space-y-1 min-w-0 flex-1">
                          <h4 className="font-black text-sm text-white tracking-wide">{b.name}</h4>
                      ...
                      <div className="space-y-2.5 text-[10px] pt-0.5">
                        <div className="flex items-start gap-2.5 text-zinc-400">
                          ...
                          <p className="leading-normal"><span className="text-zinc-500 font-bold uppercase tracking-wider text-[9px] block mb-0.5">Ecosystem Requirement:</span>{b.condition}</p>
                        </div>
                        <div className="flex items-start gap-2.5 text-zinc-400">
                          <Calendar className="h-4 w-4 text-emerald-400 flex-shrink-0 mt-0.5" />
                          <p className="leading-normal"><span className="text-zinc-500 font-bold uppercase tracking-wider text-[9px] block mb-0.5">Ecosystem Authorization:</span>{b.dateObtained}</p>
                        </div>
                      </div>
                    </div>
                  </div>
              ...
              >
                {displayBadges.map((b) => (
                  <div
                    key={b.id}
                    className="group relative flex items-center gap-2.5 p-2 rounded-xl transition-all duration-200 cursor-help border bg-[#0d111d]"
                    style={{ borderColor: "rgba(255,255,255,0.03)" }}
                    onMouseEnter={(e) => {
                      e.currentTarget.style.borderColor = b.borderColor;
                      e.currentTarget.style.backgroundColor = b.glowColor + "05";
                  ...
                  >
                    <div
                      className="bg-[#121624] border border-white/5 rounded-full flex items-center justify-center flex-shrink-0 transition-all duration-300 w-8 h-8 p-1"
                      onMouseEnter={(e) => { e.currentTarget.style.boxShadow = `0 0 10px ${b.glowColor}`; }}
                      onMouseLeave={(e) => { e.currentTarget.style.boxShadow = "none"; }}
                    >
                      <img src={b.icon} alt={b.name} className="w-full h-full object-contain filter drop-shadow-[0_1px_2px_rgba(0,0,0,0.4)]" />
                    ...
                    <div className="min-w-0 flex-1">
                      <p className="font-semibold text-zinc-300 tracking-wide truncate text-[11px] group-hover:text-white transition-colors">
                        {b.name}
                      </p>
                    </div>

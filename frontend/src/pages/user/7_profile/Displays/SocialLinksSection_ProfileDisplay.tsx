...
export const SocialLinksSectionSkeleton: React.FC = () => (
  <div className="rounded-2xl border border-white/10 bg-[#0b0e17]/60 p-4 space-y-2.5 animate-pulse">
...
export const SocialLinksSection_ProfileDisplay: React.FC<SocialLinksProps> = ({
  ...
  return (
    <div className="rounded-2xl border border-white/10 bg-[#0b0e17]/60 backdrop-blur-md p-4 text-zinc-300 shadow-xl font-['Plus Jakarta Sans',sans-serif] space-y-3 relative h-fit w-full">
          ...
          {isEditing ? (
            <>
              <button
                ...
                className="p-1.5 rounded-lg border border-white/5 bg-white/[0.02] text-zinc-400 hover:bg-red-500/10 hover:text-red-400 transition-all duration-200"
            ...
            onSaveLinks && (
              <button
                ...
                className="p-1.5 rounded-lg border border-white/5 bg-white/[0.02] text-zinc-400 hover:bg-white/10 hover:text-white transition-all duration-200 group/btn"
            ...
            >
                ...
                <div key={idx} className="flex items-center gap-2 w-full">
                  ...
                  <div className="relative flex-shrink-0">
                    <select
                      ...
                      className="appearance-none bg-[#121624] text-xs text-white font-bold px-3 py-2 pr-8 rounded-xl border border-white/5 focus:outline-none focus:border-blue-500/40 cursor-pointer h-[38px] transition-colors"
                    >
                  ...
                  <input
                    ...
                    className="flex-1 bg-white/[0.01] border border-white/5 rounded-xl px-3 py-2 text-xs font-medium text-zinc-200 placeholder-zinc-600 focus:outline-none focus:border-blue-500/40 focus:bg-white/[0.02] h-[38px] transition-all min-w-0"
                  />

                  {/* Purge Matrix Log Action Entry */}
                  <button
                    onClick={() => handleRemoveLink(idx)}
                    className="p-2.5 rounded-xl border border-white/5 bg-white/[0.01] hover:bg-red-500/10 hover:border-red-500/20 text-zinc-500 hover:text-red-400 transition-all duration-200 flex-shrink-0 h-[38px] w-[38px] flex items-center justify-center"
                  >
              ...
              {editLinks.length < 4 ? (
                <button
                  ...
                  className="flex items-center justify-center gap-2 w-full rounded-xl border border-dashed border-white/10 bg-white/[0.01] p-2.5 hover:border-white/20 hover:bg-white/[0.02] text-xs font-bold text-zinc-400 hover:text-white transition-all duration-250 cursor-pointer"
                >
                  <Plus className="h-3.5 w-3.5" />
                  <span>Attach Integration Link ({editLinks.length}/4)</span>
                </button>
              ) : (
                <div className="text-center text-[10px] text-zinc-500 font-medium italic py-1 bg-white/[0.01] border border-white/5 rounded-xl">
            ...
            >
              {!socialLinks || socialLinks.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-4 px-4 bg-white/[0.01] border border-dashed border-white/5 rounded-xl text-center space-y-1">
                  ...
                  <p className="text-[11px] text-zinc-500 font-medium">No custom link routing coordinates attached.[cite: 4]</p>
                </div>
              ) : (
                socialLinks.map((link, idx) => {
                  const match = PLATFORM_MAP[link.platform.toLowerCase()];
                  const Icon = match ? match.icon : Link2;
                  const colorClass = match ? match.color : "text-zinc-400";
                  ...
                  return (
                    <a
                      ...
                      className="group flex items-center gap-2.5 rounded-xl border border-white/[0.03] bg-white/[0.01] p-2 h-[42px] transition-all duration-300 hover:border-white/10 hover:bg-white/[0.02] w-full box-border"
                    >
                      <div className="bg-[#121624] border border-white/5 rounded-full p-1.5 flex items-center justify-center flex-shrink-0 w-6 h-6 transition-colors group-hover:bg-[#161b2c]">
                        <Icon className={`h-3.5 w-3.5 flex-shrink-0 ${colorClass}`} />
                      </div>
                      <span className="text-[11px] font-semibold text-zinc-400 truncate flex-1 group-hover:text-white transition-colors group-hover:underline">
                        {link.url}
                      </span>
                    </a>
                  );

...
export const SkillsSideSectionSkeleton: React.FC = () => (
  <div className="rounded-2xl border border-white/10 bg-[#0b0e17]/60 p-4 space-y-3 animate-pulse">
...
export const SkillsSideSection_ProfileDisplay: React.FC<SkillsSectionProps> = ({
  ...
  return (
    ...
    <div className="rounded-2xl border border-white/10 bg-[#0b0e17]/60 backdrop-blur-md p-4 text-zinc-300 shadow-xl font-['Plus Jakarta Sans',sans-serif] space-y-3 relative z-10">

      {/* Header Panel */}
      <div className="flex items-center justify-between border-b border-white/5 pb-2">
        <div className="flex items-center gap-2">
          <h3 className="text-[11px] font-extrabold text-white uppercase tracking-wider">Skills & Capabilities</h3>
        </div>
        ...
        <div className="flex items-center gap-1.5 relative">
          ...
          <div className="relative group/help">
            ...
            <div className="absolute left-full top-1/2 -translate-y-1/2 ml-3 w-72 p-3 rounded-xl border border-white/10 bg-[#070913] shadow-[0_15px_35px_rgba(0,0,0,0.6)] opacity-0 scale-95 pointer-events-none transition-all duration-200 group-hover/help:opacity-100 group-hover/help:scale-100 z-[9999] text-left origin-left">
              <div className="absolute right-full top-1/2 -translate-y-1/2 border-[5px] border-transparent border-r-[#070913]" />
              <div className="absolute right-full top-1/2 -translate-y-1/2 -mr-[1px] border-[5px] border-transparent border-r-white/10 -z-10" />
      ...
      {!skills || skills.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-4 px-4 bg-white/[0.01] border border-dashed border-white/5 rounded-xl text-center space-y-1">
          ...
          <p className="text-[11px] text-zinc-500 font-medium">No capability matrix sets created.</p>
        </div>
      ) : (
                ...
                {skills.map((skill) => {
                  ...
                  return (
                    <div
                      ...
                      className="group relative flex items-center justify-center bg-[#121420]/60 px-3 py-1 rounded-lg border transition-all duration-300 box-border overflow-hidden select-none"
                      ...
                      onMouseLeave={(e) => {
                        e.currentTarget.style.background = 'rgba(18,20,32,0.6)';
                      }}
                    >
                      <span className="text-[11px] font-bold text-zinc-200 tracking-wide transition-colors group-hover:text-white relative z-10 whitespace-nowrap">
                        {skill.name}
                      </span>
                    </div>
                  );
                  ...
                  {skills.map((skill) => {
                    ...
                    return (
                      <div
                        ...
                        className="group relative flex flex-col justify-between bg-[#121420]/60 p-2.5 rounded-xl border border-white/5 transition-all duration-300 w-full box-border overflow-hidden"
                        onMouseMove={(e) => {
                          const rect = e.currentTarget.getBoundingClientRect();
                          const x = e.clientX - rect.left;
                          const y = e.clientY - rect.top;
                      ...
                      >
                        ...
                        <div className="flex items-center justify-between gap-2 relative z-10">
                          <span className="text-xs font-bold text-white tracking-wide truncate flex-1">
                            {skill.name}
                          </span>
                          <span

...
export const OrderCardSkeleton: React.FC<{ viewType?: ViewType }> = ({
  viewType = "list",
}) => {
  return (
...
export const OrdersList: React.FC<OrdersListProps> = ({
  ...
  if (loading) {
    return (
      ...
      >
        {[1, 2, 3, 4, 5, 6].map((i) => (
          <OrderCardSkeleton key={i} viewType={viewType} />
        ))}
      </div>
    );
  }
  ...
  if (orders.length === 0) {
    return (
      <div className="rounded-2xl border border-gray-200 dark:border-white/10 bg-white dark:bg-dark-surface shadow-sm dark:shadow-none p-12 text-center">
        <p className="text-sm text-gray-500 dark:text-zinc-400 font-medium">No orders found matching your criteria.</p>
      </div>
    );
  }
          ...
          return (
            <motion.div
              ...
              onClick={() =>
                navigate(
                  item.type === "incoming"
                    ? `/gigs/orders/received/${item.id}`
                    : `/gigs/orders/sent/${item.id}`
                )
              }
            ...
            >
              <div className="space-y-3.5">
                {/* 1. User Header & Clickable Target Gig Post Link */}
                <div className={`flex ${viewType === "grid" ? "flex-col-reverse gap-3" : "items-start justify-between gap-3"} border-b border-gray-100 dark:border-white/5 pb-3`}>
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="flex items-center gap-1 text-xs text-gray-500 dark:text-zinc-400 truncate">
                        ...
                        <button
                          ...
                          onClick={(e) => {
                            e.stopPropagation();
                            navigate(`/gigs/postings/${item.gigId}`);
                          }}
                          ...
                          title="View Gig Post Details"
                        >
                          <span className="truncate">{item.gigTitle}</span>
                  ...
                  <div className={`flex items-center gap-2 shrink-0 ${viewType === "grid" ? "w-full justify-between" : ""}`}>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {/* Edit Button for Own Orders */}
                      {item.type === "sent" && item.status === "Pending" && (
                        <button
                          title="Edit Order"
                          onClick={(e) => {
                            e.stopPropagation();
                            navigate(`/gigs/orders/edit/${item.id}`);
                          }}
                ...
                {viewType === "list" && (
                  <div className="px-1 text-gray-600 dark:text-zinc-300 italic">
                    <GigRichText content={item.coverLetter} truncate={2} />
                  </div>
                )}

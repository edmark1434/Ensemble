// src/pages/user/4_forums/forum_modals/ReportGroupModal.tsx
...
const ReportGroupModal: React.FC<ReportGroupModalProps> = ({
  isOpen,
  onClose,
  groupName,
  onSubmit,
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm animate-fade-in-modal">
      <div className="w-full max-w-md rounded-2xl border border-white/10 bg-[#0d0f1a] p-6 shadow-2xl animate-scale-in">
        ...
        <p className="text-sm text-zinc-400 mb-4">
          Reporting <span className="text-white font-medium">{groupName}</span>
        </p>
        ...
        <div className="mb-6">
          ...
          <textarea
            ...
            placeholder="Please provide more details about why you're reporting this group..."
            rows={4}
            className={`w-full rounded-lg border ${
              errors.description ? "border-red-500/50" : "border-white/15"
            } bg-white/5 px-4 py-2.5 text-sm text-white placeholder:text-zinc-500 focus:border-blue-500/50 focus:outline-none focus:ring-1 focus:ring-blue-500/50 resize-none`}
            style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}
          />
          {errors.description && (
            <p className="mt-1 text-xs text-red-400 flex items-center gap-1">

...
const SuccessModal: React.FC<SuccessModalProps> = ({
  ...
  // Automatically handle the loading to success sequence transition whenever the modal mounts or opens
  useEffect(() => {
    if (isOpen) {
      setStatus("loading");
      const timer = setTimeout(() => {
        setStatus("success");
      }, 1800); // 1.8 seconds processing delay simulation
  ...
  return (
    <div className="fixed inset-0 z- flex items-center justify-center p-4 bg-black/85 backdrop-blur-md animate-fade-in overflow-hidden">
      ...
      {/* ========================================================================= */}
      {/* GLOBAL FULL-SCREEN CONFETTI CANVAS (MOVED ABOVE & OUTSIDE THE MAIN CARD)  */}
      {/* ========================================================================= */}
      {status === "success" && (
        <div className="absolute inset-0 pointer-events-none w-full h-full z-30">
          {[...Array(30)].map((_, i) => {
            ...
            const inlineStyles = {
              "--left": `${15 + Math.random() * 70}%`, // Wider distribution range across the screen
              "--tx": `${(Math.random() - 0.5) * 450}px`, // Increased horizontal blast range
              "--ty": `${-180 - Math.random() * 250}px`, // Increased upward blast peak height
      ...
      <div className="relative w-full max-w-sm rounded-3xl border border-white/10 bg-[#0d0f1a]/90 p-8 text-center shadow-2xl min-h-[320px] flex flex-col justify-center items-center backdrop-blur-xl z-10 overflow-hidden">
        ...
        {status === "loading" ? (
          /* ======================================= */
          /* STATE A: SIMULATED PROCESSING VIEW      */
          /* ======================================= */
          ...
          <div className="animate-scale-up flex flex-col items-center w-full relative">
            ...
            {/* Modal Typographic Content Mapping */}
            ...
            {/* Dynamic Confirmation Button Endpoint Action Handler */}
            <button
              onClick={onConfirm}
      ...
      {/* --- CONFETTI EXPLOSION CSS KEYFRAMES --- */}
      <style>{`

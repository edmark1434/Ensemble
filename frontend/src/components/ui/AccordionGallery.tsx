    ...
    (animate: boolean) => {
      ...
      panels.forEach((panel, i) => {
        ...
        if (showLabels && bar && text) {
          const elements = desc ? [bar, text, desc] : [bar, text];
          if (isActive) {
            tl.to(elements, { opacity: 1, x: 0, duration: dur, ease, stagger: prefersReduced ? 0 : stagger }, 0);
          } else {
            tl.to(elements, { opacity: 0, x: -14, duration: dur * 0.6, ease }, 0);
          }
        }
      });
            ...
            {showLabels && (
              ...
              >
                <span
                  ...
                  className="h-[46px] w-[3px] flex-none rounded-[3px] opacity-0"
                  style={{

...
const ScrollExpand: React.FC<ScrollExpandProps> = ({
  ...
  const applyProgress = useCallback((p: number) => {
    ...
    let rawE = 0;
    if (p <= c.scrollDistance) {
      rawE = p / (c.scrollDistance || 1);
    } else if (p <= c.scrollDistance + c.holdDistance) {
      rawE = 1;
    } else {
      const shrinkStart = c.scrollDistance + c.holdDistance;
      const shrinkProgress = (p - shrinkStart) / (c.shrinkDistance || 1);
      rawE = 1 - shrinkProgress;
    }
    rawE = clamp(rawE, 0, 1);
    const e = smoothstep(0, 1, rawE);
    ...
    if (titleRef.current) {
      const out = smoothstep(0.4, 0.88, rawE);
      titleRef.current.style.opacity = `${1 - out}`;
      titleRef.current.style.transform = `translate3d(0, ${-28 * out}px, 0) scale(${1 + 0.06 * out})`;
    }

    if (hintRef.current) {
      const gone = smoothstep(0, 0.12, rawE);
      hintRef.current.style.opacity = `${1 - gone}`;
      hintRef.current.style.transform = `translate3d(0, ${8 * gone}px, 0)`;
    }

    if (overlayRef.current) {
      const inn = smoothstep(0.68, 1, rawE);
  ...
  useEffect(() => {
    ...
    const measure = () => {
      ...
      track.style.height = `${stageH * (1 + Math.max(0, c.scrollDistance) + Math.max(0, c.holdDistance) + Math.max(0, c.shrinkDistance))}px`;
    ...
    const readProgress = () => {
      ...
      const totalDist = c.scrollDistance + c.holdDistance + c.shrinkDistance;
      if (totalDist <= 0) return 1;
      
      if (c.useWindowScroll) {
        const top = track.getBoundingClientRect().top;
        return clamp(-top / stageH, 0, totalDist);
      }
      return clamp(root.scrollTop / stageH, 0, totalDist);
    };
  ...
  useEffect(() => {
    if (mediaType === 'video' && mediaRef.current && Array.isArray(src)) {
      const video = mediaRef.current as HTMLVideoElement;
      let currentIndex = 0;
      let timer: NodeJS.Timeout;

      const playNext = () => {
        video.src = src[currentIndex];
        video.play().catch(() => {});

...
const TargetCursor: FC<TargetCursorProps> = ({
  ...
  useEffect(() => {
    ...
    const enterHandler = (e: MouseEvent) => {
      ...
      activeTarget = target;
      const corners = Array.from(cornersRef.current);
      corners.forEach(corner => gsap.killTweensOf(corner, { x: true, y: true }));

      gsap.killTweensOf(cursorRef.current, { rotation: true });
      ...
      const leaveHandler = () => {
        ...
        if (cornersRef.current) {
          ...
          gsap.killTweensOf(currentCorners, { x: true, y: true });
